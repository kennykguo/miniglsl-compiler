...
int semantic_check(node *ast) {
    ...
    add_predefined("gl_FragCoord", VEC_T, 3, 0, 1, 0);
...
static int check_statement(node *stmt) {
    ...
    switch(stmt->kind) {
        case ASSIGNMENT_NODE: {
            ...
            // STORE THE VARIABLE'S TYPE IN THE AST NODE - ADD THIS
            stmt->assignment.variable->result_type = var->type_code;
            stmt->assignment.variable->result_vec_size = var->vec_size;
            ...
            if (check_expression(stmt->assignment.expr)) {
                int expr_type, expr_size;
                expr_type = get_expression_type(stmt->assignment.expr, &expr_size);
                
                if (!types_match(var->type_code, var->vec_size, expr_type, expr_size)) {
    ...
    switch(expr->kind) {
        case VAR_NODE: {
            ...
            if (expr->variable.is_array) {
                int max_index = var->vec_size + 1;
                if (expr->variable.array_index < 0 || expr->variable.array_index > max_index) {
