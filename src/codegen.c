...
if (!stmts) return;
process_stmts_codegen(stmts->list.next, out);
gen_statement(stmts->list.item, out);
}
...
switch (stmt->kind) {
    ...
    case IF_STMT_NODE: {
        ...
        // handle simple case: if (cond) { x = a; } else { x = b; }
        if (stmt->if_stmt.then_stmt && 
            ...
            // select between values based on condition (PREDICATION!)
            const char *var_type = lookup_var_type(var_name);
            if (!var_type) {
                const char *builtin = get_builtin_var(var_name);
                var_type = builtin ? "%t_vec4" : "%t_vec4";
            }
            ...
            // for scopes, we process statements and use OpSelect for each assignment
            node *then_scope = stmt->if_stmt.then_stmt;
            node *else_scope = stmt->if_stmt.else_stmt;
            
            // For complex scopes, we evaluate all expressions in both branches
            // and use OpSelect for each result. This is tricky and may need
            // special handling depending on the shader structure.
            fprintf(stderr, "warning - complex if/else scopes with predication\n");
            
            // Process both branches (expressions get evaluated)
            if (then_scope->kind == SCOPE_NODE) {
                process_stmts_codegen(then_scope->scope.statements, out);
            }
            if (else_scope && else_scope->kind == SCOPE_NODE) {
                process_stmts_codegen(else_scope->scope.statements, out);
            }
        }
        break;
    }
    ...
    case SCOPE_NODE: {
        ...
        process_stmts_codegen(stmt->scope.statements, out);
        break;
    }
